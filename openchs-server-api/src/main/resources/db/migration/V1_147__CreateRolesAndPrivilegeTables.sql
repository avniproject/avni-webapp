...
from operational_subject_type s
         join groups g on g.organisation_id = s.organisation_id
...
from operational_program op
         join form_mapping fm on fm.entity_id = op.program_id
         join form f on fm.form_id = f.id
         join groups g on g.organisation_id = op.organisation_id
         join privilege p on true
where p.name in ('Enrol subject', 'View enrolment details', 'Edit enrolment details', 'Exit enrolment')
  and fm.entity_id notnull
  and fm.is_voided = false
  and f.form_type in ('ProgramEnrolment');
...
from operational_encounter_type oet
         join form_mapping fm on fm.observations_type_entity_id = oet.encounter_type_id
         join form f on fm.form_id = f.id
         join groups g on g.organisation_id = oet.organisation_id
         join privilege p on true
where p.name in ('View visit', 'Schedule visit', 'Perform visit', 'Edit visit', 'Cancel visit')
  ...
  and fm.is_voided = false
  and f.form_type in ('ProgramEncounter', 'Encounter');
...
select uuid_generate_v4(),
       ...
       true
from operational_subject_type s
         join checklist_detail cd on s.organisation_id = cd.organisation_id
         join groups g on g.organisation_id = s.organisation_id
         join privilege p on true
