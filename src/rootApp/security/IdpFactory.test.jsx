...
it("when not both", function() {
  ...
  assert.isTrue(
    IdpFactory.createIdp(IdpDetails.keycloak, {}) instanceof KeycloakWebClient
  );
  assert.isTrue(
    IdpFactory.createIdp(IdpDetails.cognito, {}) instanceof CognitoWebClient
  );
  assert.isTrue(
    IdpFactory.createIdp(IdpDetails.both, {}) instanceof UndecidedIdp
  );
});
...
it("when both but with local storage having keycloak only item", function() {
  ...
  assert.isTrue(
    IdpFactory.createIdp(IdpDetails.both, {}) instanceof KeycloakWebClient
  );
});
...
it("when both but with local storage having cognito item", function() {
  ...
  assert.isTrue(
    IdpFactory.createIdp(IdpDetails.both, {}) instanceof CognitoWebClient
  );
});
