        ...
        {searchFilterForms.map((searchFilterForm, index) => {
          const selectedValue = _.head(
            selectedConcepts.filter(
              (c) => c.conceptUUID === searchFilterForm.conceptUUID,
            ),
          );
          let selected = {};
          selectedValue &&
            _.forEach(selectedValue.values, (sv) =>
              _.assign(selected, { [sv]: true }),
            );
                ...
                {conceptList.map((concept, index) =>
                  concept.uuid === searchFilterForm.conceptUUID
                    ? concept.conceptAnswers
                        .filter(
                          (conceptAnswer) =>
                            !conceptAnswer.voided &&
                            !conceptAnswer.answerConcept?.voided,
                        )
                        .map((conceptAnswer, index) => (
                          <FormControlLabel
                            control={
                              <Checkbox
                                checked={
                                  selected != null
                                    ? Boolean(
                                        selected[
                                          conceptAnswer.answerConcept.uuid
                                        ],
                                      )
                                    : false
                                }
