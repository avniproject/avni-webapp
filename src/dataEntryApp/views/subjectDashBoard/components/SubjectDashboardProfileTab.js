...
const SubjectDashboardProfileTab = ({
  ...
  function renderSubjectProfile() {
    return (
      <StyledAccordion>
        <StyledAccordionSummary expandIcon={<StyledExpandMore />} aria-controls="registrationPanelbh-content" id="profile-detail">
        ...
        <StyledAccordionDetails>
          <Grid size={12}>
            ...
            <StyledButton color="primary" onClick={() => setVoidConfirmation(true)}>
              {t("void")}
            </StyledButton>
  ...
  function renderRelatives() {
    return (
      <StyledAccordion>
        <StyledAccordionSummary expandIcon={<StyledExpandMore />} aria-controls="relativesPanelbh-content" id="relativesPanelbh-header">
          <StyledTypographyHeading component="span">{t("Relatives")}</StyledTypographyHeading>
        </StyledAccordionSummary>
        <StyledAccordionDetails>
          {profile.relationships !== undefined && relativeList.length !== 0 ? (
            ...
            <StyledTypographyNoItems variant="caption">{t("noRelativesAdded")}</StyledTypographyNoItems>
          )}
        </StyledAccordionDetails>
  ...
  function renderGroupMembers() {
    return (
      <StyledAccordion>
        ...
        >
          <StyledTypographyHeading component="span">{t("members")}</StyledTypographyHeading>
        </StyledAccordionSummary>
          ...
          {profile.roles && profile.roles.length > 0 ? (
            sortBy(profile.roles, [profileRole => profileRole.role]).map((profileRole, index) => (
              <StyledAccordion key={index} defaultExpanded>
                ...
                >
                  <StyledTypographyHeading component="span">{t(profileRole.role)}</StyledTypographyHeading>
                </StyledAccordionSummary>
                <StyledAccordionDetails>
                  <GridCardView
            ...
            <StyledTypographyNoItems variant="caption">{t("noGroupMembersAdded")}</StyledTypographyNoItems>
          )}
        </StyledGroupMembersAccordionDetails>
        <StyledButton color="primary">
  ...
  function renderGroupMemberships() {
    const groupMembershipCards = profile.memberships.map(membership => (
      <GroupMembershipCardView key={membership.uuid} groupMembership={membership} />
    ));
    return (
      <StyledAccordion>
        <StyledAccordionSummary
        ...
        >
          <StyledTypographyHeading component="span">{t("Memberships")}</StyledTypographyHeading>
        </StyledAccordionSummary>
        <StyledAccordionDetails>
    ...
    <Fragment>
      {profile && profile.voided ? (
        <StyledPaper>
          ...
          {!_.isEmpty(unVoidErrorKey) && <StyledTypographyError variant="button">{t(unVoidErrorKey)}</StyledTypographyError>}
          {renderDialog(
            "Un-Void the subject",
            unVoidConfirmation,
