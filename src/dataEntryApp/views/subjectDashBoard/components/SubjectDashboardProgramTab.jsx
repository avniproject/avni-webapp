...
const SubjectDashboardProgramTab = ({
  ...
  if (program && program.enrolments) {
    program.enrolments.sort((left, right) =>
      left.programExitDateTime ? 1 : right.programExitDateTime ? -1 : 0
    );
  }
        ...
        <StyledProgramBar>
          <Grid container spacing={1}>
            ...
            {!(program && program.enrolments) && (
              <StyledTypographyNoEnrolments component="span">
                {t("notEnroledInAnyProgram")}
              </StyledTypographyNoEnrolments>
            )}
          </Grid>
        </StyledProgramBar>
        {selectedTab !== false ? (
