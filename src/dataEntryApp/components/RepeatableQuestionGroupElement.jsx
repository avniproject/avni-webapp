...
function AddMoreButton({ addNewQuestionGroup, formElement }) {
  return (
    <Button
      onClick={() => addNewQuestionGroup(formElement)}
      color="primary"
...
function RemoveButton({ removeQuestionGroup, formElement, index }) {
  return (
    <Button
      onClick={() => removeQuestionGroup(formElement, index)}
      color="primary"
    >
      {"Remove Above"}
    </Button>
...
export function RepeatableQuestionGroupElement({
  ...
  const oneOfTheQuestionGroupObservationsIsEmpty = _.some(
    repeatableQuestionGroupValue,
    x => _.isEmpty(x.groupObservations)
  );
  return repeatableQuestionGroupValue.map((x, index) => {
    const isLastElement =
      !hasNoObservation &&
      !oneOfTheQuestionGroupObservationsIsEmpty &&
      repeatableQuestionGroupValue.length === index + 1;
    const quesGrpValidationResults = validationResults.filter(
      itr => itr.questionGroupIndex === index
    );
    return (
      <div key={index}>
