...
export default function PhoneNumberFormElement({
  ...
  const phoneNumber = isNil(observation)
    ? new PhoneNumber()
    : observation.getValueWrapper();
  const isVerified = phoneNumber.isVerified();
  const isUnverified =
    get(validationResult, "success", true) &&
    !isVerified &&
    size(phoneNumber.getValue()) === 10;
      ...
      <FormControlLabel
        control={
          <Checkbox
            checked={isVerified}
            onChange={event =>
              update({ phoneNumber: number, isVerified: event.target.checked })
            }
            name="verified"
            color="primary"
          />
  ...
  const renderUnverifiedMessage = () => {
    return (
      <StyledError variant="body1">{t("phoneNumberUnverified")}</StyledError>
    );
  };
  ...
  return (
    <div>
      ...
      <StyledTextField
        ...
        name={name}
        value={
          isNaN(parseInt(phoneNumber.getValue())) ? "" : phoneNumber.getValue()
        }
        helperText={
          validationResult &&
        ...
        onChange={e => {
          ...
          isEmpty(phoneNumber)
            ? update({})
            : update({ phoneNumber, isVerified: false });
        }}
        disabled={!editable}
      />
      {renderVerifyOption()}
