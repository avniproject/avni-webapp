...
const fetchFilteredFormElementsAndUpdateEntityObservations = (
  formElementGroup,
  entity
) => {
  const obsHolder = new ObservationsHolder(entity.observations);
  const { filteredFormElements, formElementStatuses } = getFormElementStatuses(
    entity,
    formElementGroup,
    obsHolder
  );
  ...
  if (
    hasQuestionGroupWithValueInElementStatus(
      formElementStatuses,
    ...
    } = getFormElementStatuses(entity, formElementGroup, obsHolder);
    return filteredFormElementsLatest;
  }
  return filteredFormElements;
};

const getUpdatedNextFilteredFormElements = (
  formElementStatuses,
  nextGroup,
  entity,
  ...
  if (
    hasQuestionGroupWithValueInElementStatus(
      formElementStatuses,
      nextGroup.getFormElements()
    )
  ) {
    let {
      filteredFormElements: updatedNextFilteredFormElements
...
const hasQuestionGroupWithValueInElementStatus = (
  formElementStatuses,
  allFormElements
) => {
...
const onLoad = (
  ...
  const getReturnObject = (formElementGroup, entity, isSummaryPage = false) => {
    const indexOfGroup =
      findIndex(
        ...
        feg => feg.uuid === formElementGroup.uuid
      ) + 1;
    const filteredFormElements = fetchFilteredFormElementsAndUpdateEntityObservations(
      formElementGroup,
      entity
    );
    return {
  ...
  const formElementGroup = isImmutable
...
const errors = validationResults =>
  filter(validationResults, isFailedValidation);

const getIdValidationErrors = (filteredFormElements, obsHolder) => {
  const isIdFieldWithoutObservation = fe =>
    fe.getType() === Concept.dataType.Id &&
    isNil(obsHolder.findObservation(fe.concept));
  const createValidationResult = fe =>
    ValidationResult.failure(fe.uuid, "ranOutOfIds");

  return filter(filteredFormElements, isIdFieldWithoutObservation).map(
    createValidationResult
  );
};

// Utility functions for form element validation
const getStandaloneElements = formElements =>
  filter(
    formElements,
    fe => !isNil(fe) && !isNil(fe.concept) && isNil(fe.group)
  );
...
const getQuestionGroupElements = formElements =>
  filter(
    ...
    fe =>
      !isNil(fe) &&
      !isNil(fe.concept) &&
      fe.concept.datatype === Concept.dataType.QuestionGroup
  );

const getChildFormElements = formElements =>
  filter(
    formElements,
    fe => !isNil(fe) && !isNil(fe.concept) && !isNil(fe.group)
  );
...
const validateElement = ({
  ...
  const obsValue = groupObservation
    ? groupObservation.getObservation(formElement.concept)
    : obsHolder.findObservation(formElement.concept);
 ...
 * @returns {Array} Validation results
 */
const validateChildInRepeatableGroup = ({
  childFormElement,
  obsHolder,
  formElementStatuses,
  validationResults
  ...
  const indexProcessed = some(
    results,
    vr =>
      vr.formIdentifier === childFormElement.uuid &&
      vr.questionGroupIndex === childFormElement.questionGroupIndex
  );
  ...
  if (!indexProcessed) {
    ...
    if (!isNil(questionGroupWrapper)) {
      const groupObservation = questionGroupWrapper.getGroupObservationAtIndex(
...
const getFEDataValidationErrors = (filteredFormElements, obsHolder) => {
  ...
  childElements.forEach(childFormElement => {
    ...
    const alreadyProcessed = some(
      validationResults,
      vr => vr.formIdentifier === childFormElement.uuid
    );

    // For repeatable question groups, also check if the specific questionGroupIndex was processed
    const isInRepeatableGroup =
      !isNil(childFormElement.group) && childFormElement.group.repeatable;
...
const onNext = ({
  ...
  const ruleValidationErrors = filterFormElementStatusesAndConvertToValidationResults(
    formElementGroup,
    entity
  );
  ...
  const idValidationErrors = getIdValidationErrors(
    filteredFormElements,
    obsHolder
  );
  const dataValidationErrors = getFEDataValidationErrors(
    filteredFormElements,
    obsHolder
  );

  // Filter validation errors using intersectionBy to only include those that match form elements in ruleValidationErrors
  const filteredIdValidationErrors = intersectionBy(
    idValidationErrors,
    ruleValidationErrors,
    "formIdentifier"
  );
  const filteredDataValidationErrors = intersectionBy(
    dataValidationErrors,
    ruleValidationErrors,
    "formIdentifier"
  );
  const filteredValidationResults = intersectionBy(
    validationResults || [],
    ruleValidationErrors,
    "formIdentifier"
  );
  const filteredEntityValidations = intersectionBy(
    entityValidations || [],
    ruleValidationErrors,
    "formIdentifier"
  );
  ...
  const anyFailedResultForCurrentFEG = () => {
    const formUUIDs = union(
      formElementGroup.formElementIds,
      staticFormElementIds
    );
  ...
  const {
    filteredFormElements: nextFilteredFormElements,
    formElementStatuses
  } = !isEmpty(nextGroup)
    ? filterFormElementsWithStatus(nextGroup, entity)
  ...
  if (isEmpty(nextFilteredFormElements)) {
    ...
    obsHolder.updatePrimitiveCodedObs(
      nextFilteredFormElements,
      formElementStatuses
    );
    ...
    return nextState(
      nextGroup,
      updatedNextFilteredFormElements,
      allRuleValidationResults,
      obsHolder.observations,
      entity,
      false,
...
const onPrevious = ({
  ...
  const {
    filteredFormElements: previousFilteredFormElements,
    formElementStatuses
  } = !isEmpty(previousGroup)
  ...
  if (isEmpty(previousFilteredFormElements)) {
    obsHolder.removeNonApplicableObs(
      previousGroup.getFormElements(),
  ...
  newValidationResults.forEach(newValidationResult => {
    remove(
      ...
      existingValidationResult =>
        ...
        newValidationResult.formIdentifier
    );
...
function postObservationsUpdate(
  ...
  let { formElementStatuses, filteredFormElements } = getFormElementStatuses(
    entity,
    formElement.formElementGroup,
    observationsHolder
  );
  observationsHolder.updatePrimitiveCodedObs(
    filteredFormElements,
    formElementStatuses
  );
  if (
    hasQuestionGroupWithValueInElementStatus(
      formElementStatuses,
      formElement.formElementGroup.getFormElements()
    )
  ) {
    const {
      filteredFormElements: filteredFormElementsLatest
...
function addNewQuestionGroup(entity, formElement, observations) {
  const repeatableQuestionGroup = getRepeatableQuestionGroup(
    observations,
    formElement.concept
  );
  repeatableQuestionGroup.addQuestionGroup();
  return getFormElementStatuses(
    entity,
    formElement.formElementGroup,
...
function removeQuestionGroup(
  ...
  if (existingValidationResults && existingValidationResults.length > 0) {
    ...
    const shouldKeepValidationResult = validationResult => {
      return !(
        ...
        formElement.formElementGroup
          .getFormElements()
          .some(fe => fe.uuid === validationResult.formIdentifier)
      );
    };
  ...
  const { filteredFormElements } = getFormElementStatuses(
    entity,
    formElement.formElementGroup,
    new ObservationsHolder(observations)
  );
...
const getValidationResult = (validationResults, formElementIdentifier) =>
  find(
    validationResults,
    validationResult =>
      validationResult.formIdentifier === formElementIdentifier
  );
