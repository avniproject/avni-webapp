  ...
  switch (action.type) {
    ...
    case types.SET_LEGACY_RULES_BUNDLE: {
      ...
      if (!isEmpty(action.rulesBundle)) {
        try {
          const executeBundle = new Function(action.rulesBundle);
          executeBundle();

          const getRulesConfig = new Function('return typeof rulesConfig !== "undefined" ? rulesConfig : {};');
          const result = getRulesConfig();
