...
export function SubjectTypeSelect({
  isMulti,
  onChange,
  ...
  useEffect(() => {
    httpClient.getAllData("subjectType", "/web/subjectType").then(response => {
      const namedSelectableEntities = NamedSelectableEntities.create(
        WebSubjectType.fromResources(response),
        selectedSubjectTypes
      );
      ...
      >
        <div style={{ width: "300px", marginRight: 10 }}>
          <Select
            ...
            onChange={e => {
              onChange(
                subjectTypes.toggle(selectedSubjectTypes, e.value, isMulti)
              );
            }}
            maxMenuHeight={200}
          />
