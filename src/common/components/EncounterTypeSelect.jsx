...
export function EncounterTypeSelect({
  isMulti,
  ...
  useEffect(() => {
    ...
    const programParam =
      selectedPrograms.length > 0
        ? selectedPrograms.map(x => `program=${x.uuid}`)
        : "";
    httpClient
      .getData(`/web/encounterType/v2?${subjectTypeParam}&${programParam}`)
      .then(response => {
        setEncounterTypes(
          NamedSelectableEntities.create(
            WebEncounterType.fromResources(response)
          )
        );
      });
  }, [selectedSubjectTypes]);
      ...
      >
        <div style={{ width: "300px", marginRight: 10 }}>
          <Select
            ...
            value={encounterTypes.getSelectedValue(
              selectedEncounterTypes,
              isMulti
            )}
            options={options}
            onChange={e => {
              onChange(
                encounterTypes.toggle(selectedEncounterTypes, e.value, isMulti)
              );
            }}
            maxMenuHeight={200}
          />
