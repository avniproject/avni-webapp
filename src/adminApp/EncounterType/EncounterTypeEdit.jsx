...
const EncounterTypeEdit = () => {
  ...
  useEffect(() => {
    console.log(
      "useEffect for message rules, entityType:",
      entityType,
      "encounterTypeId:",
      ...
      .then(result => {
        ...
        http
          ...
          .then(response => {
            ...
            const encounterTypeMappings = formMap.filter(
              l => l.encounterTypeUUID === result.uuid
            );
            console.log(
              "Encounter type mappings for UUID",
              result.uuid,
              ":",
              encounterTypeMappings
            );

            if (encounterTypeMappings.length === 0) {
              console.warn(
                "No encounter type mappings found for UUID:",
                result.uuid
              );
              ...
              console.log(
                "Current encounterType state after dispatch:",
                encounterType
              );
              return;
            }
            ...
            setEntityType(
              _.isNil(programUUID) ? "Encounter" : "ProgramEncounter"
            );

            const subject = (response.data.subjectTypes || []).find(
            ...
            if (form) {
              ...
              console.warn(
                "No program encounter form found for encounter type:",
                result
              );
              dispatch({ type: "programEncounterForm", payload: null });
            }

            const cancellationForm = findProgramEncounterCancellationForm(
              formMap,
              result
            );
            console.log(
              "Program encounter cancellation form:",
              cancellationForm
            );
            if (cancellationForm) {
              dispatch({
                type: "programEncounterCancellationForm",
                payload: cancellationForm
              });
            } else {
              console.warn(
                "No program encounter cancellation form found for encounter type:",
                result
              );
              dispatch({
                type: "programEncounterCancellationForm",
  ...
  function updateProgram(program) {
    ...
    if (_.isEmpty(program)) {
      console.log("Program is empty, checking form types:", {
        formType,
        cancelFormType
      });
      ...
      console.log("Program is set, checking form types:", {
        formType,
        cancelFormType
      });
      if (formType === "Encounter") {
        console.log(
          "Resetting programEncounterForm due to Encounter form type"
        );
        resetValue("programEncounterForm");
      }
      if (cancelFormType === "IndividualEncounterCancellation") {
        console.log(
          "Resetting programEncounterCancellationForm due to IndividualEncounterCancellation form type"
        );
        resetValue("programEncounterCancellationForm");
      }
    }
