...
export const ShowSubjectType = props => {
  const existMapping = props.formMapping.filter(
    l => l[props.entityUUID] === props.rowDetails.uuid
  );

  const subjectType =
    existMapping.length !== 0
      ? props.subjectType.filter(
          l => l.uuid === existMapping[0].subjectTypeUUID
        )
      : [];
  ...
  return (
    <>
      {existMapping.length !== 0 && program.length !== 0 && (
        <span>{program[0].operationalProgramName || program[0].name}</span>
      )}
