...
function WorkFlowEncounterForm(props) {
  ...
  const encounters = props.formMapping.filter(
    l => l.encounterTypeUUID === props.rowDetails.uuid
  );
  if (
    props.whichForm === "encounter" &&
  ...
  existMapping = props.formMapping.filter(
    l => l.encounterTypeUUID === props.rowDetails.uuid
  );
  ...
  showAvailableForms =
    form.length === 0
      ? props.formList.filter(
          form => form.formType === formType && form.formName !== undefined
        )
      : [];
...
export default memo(WorkFlowEncounterForm, areEqual);
