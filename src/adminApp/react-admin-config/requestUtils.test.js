...
describe("pagination", () => {
  it("stringifyPagination", () => {
    assert.equal(
      ...
      "page=3&size=20&sort=name%2CASC"
    );
    assert.equal(
      stringifyPagination({ page: 4, perPage: 20 }, [
        { field: "name", order: "ASC" }
      ]),
      "page=3&size=20&sort=name%2CASC"
    );
    assert.equal(
      ...
      "page=3&size=20&sort=name%2CASC&sort=id%2CASC"
    );
    assert.equal(
      stringifyPagination({ page: 4, perPage: 20 }, { field: "name" }),
      "page=3&size=20&sort=name"
    );
    assert.equal(
      stringifyPagination({ page: 4, perPage: 20 }, "name"),
      "page=3&size=20&sort=name"
    );
  });
});
...
describe("FilterProcessor", () => {
  it("getPath", function() {
    ...
    assert.equal(
      FP.getPath({ searchURI: "lastModified", someId: 2 }),
      "search/lastModified"
    );
  });
  ...
  it("stringifyFilter", function() {
    assert.equal(
      FP.stringifyFilter({ someId: 2, otherId: 3 }),
      "otherId=3&someId=2"
    );
    assert.equal(
      FP.stringifyFilter({ searchURI: "lastModified", someId: 2 }),
      "someId=2"
    );
  });
});
...
describe("UrlPartsGenerator", () => {
  it("forList", function() {
    ...
    assert.equal(
      UPG.forList(params),
      "search/byState?stateId=2&page=3&size=20&sort=name%2CASC"
    );
    ...
    assert.equal(
      UPG.forList(params),
      "search/findByOrganisation?organisationId=2"
    );
