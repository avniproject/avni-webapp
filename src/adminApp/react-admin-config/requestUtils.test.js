...
import { stringifyPagination, FilterProcessor as FP, UrlPartsGenerator as UPG } from "./requestUtils";
...
describe("pagination", () => {
  it("stringifyPagination", () => {
    ...
    assert.equal(stringifyPagination({ page: 4, perPage: 20 }, [{ field: "name", order: "ASC" }]), "page=3&size=20&sort=name%2CASC");
    assert.equal(
      stringifyPagination({ page: 4, perPage: 20 }, [{ field: "name", order: "ASC" }, { field: "id", order: "ASC" }]),
