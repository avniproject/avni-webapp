    ...
    () => [
      {
        ...
        enableSorting: true,
        Cell: ({ row }) => (
          <a href={`#/appDesigner/program/${row.original.id}/show`}>
            {row.original.name}
          </a>
        )
      },
        ...
        Cell: ({ row }) => (
          <a
            href={`#/appdesigner/forms/${get(
              findProgramEnrolmentForm(formMappings, row.original),
              "formUUID"
            )}`}
          >
            {get(
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <a
            href={`#/appdesigner/forms/${get(
              findProgramExitForm(formMappings, row.original),
              "formUUID"
            )}`}
          >
        ...
        Cell: ({ row }) => (
          <div
            style={{
              ...
              height: "20px",
              border: "1px solid",
              background: row.original.colour
            }}
          >
            {" "}
          </div>
        ...
        ? [
            {
              ...
              onClick: (event, row) =>
                navigate(`/appDesigner/program/${row.original.id}`),
            ...
            },
            {
              ...
              onClick: (event, row) => {
                const voidedMessage = `Do you really want to delete the program ${
                  row.original.name
                }?`;
