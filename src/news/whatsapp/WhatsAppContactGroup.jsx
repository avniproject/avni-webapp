...
const fetchData = async (query, contactGroupId, onContactGroupLoaded) => {
  const data = await ContactService.getContactGroupContacts(
    contactGroupId,
    query.page,
    query.pageSize
  );
...
function Members({
  ...
  const columns = [
    ...
    { accessorKey: "maskedPhone", header: t("maskedPhone") }
  ];
  ...
  const removeContactFromGroup = useCallback(
    contactRows => {
      ...
      ContactService.removeContactsFromGroup(
        contactGroupId,
        contactRows.map(x => x.id)
      )
        .then(() => {
          contactGroupMembersUpdated();
          setUserDeleted(true);
    ...
    <ScreenWithAppBar appbarTitle={"WhatsApp Contact Group"}>
      ...
      <Box sx={{ flexGrow: 1, bgcolor: "background.paper", display: "flex" }}>
        ...
        >
          <Tab
            label="Members"
            value={ContactGroupTabs.members}
        ...
        {selectedTab === ContactGroupTabs.members && (
          <Members
            contactGroupId={contactGroupId}
            onContactGroupLoaded={contactGroup =>
              setGroup(contactGroup["group"])
            }
            contactGroupMembersUpdated={() =>
              updateContactGroupVersion(
                contactGroupVersion => contactGroupVersion + 1
              )
            }
