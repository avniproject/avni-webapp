...
const ComposeMessageView = ({
  ...
  const onRuleChange = updatedValues => {
    if (_.has(updatedValues, "messageTemplateId")) {
      const noOfParameters = getNumberOfParametersForTemplate(
        updatedValues.messageTemplateId
      );
      const parameters = Array.apply(null, Array(noOfParameters)).map(
        function() {
          return "";
        }
      );
        ...
        <form onSubmit={onSubmit}>
          <Box style={{ marginLeft: 10 }}>
            ...
            <Select
              ...
              onChange={({ value }) =>
                onRuleChange({ messageTemplateId: value.id })
              }
            />
          </Box>
          ...
          <Box style={{ marginTop: 20, marginLeft: 10 }}>
            {rules[0].messageTemplateId && (
              <AvniFormLabel label={"Selected message template"} />
            )}
            {rules[0].messageTemplateId &&
              getSelectedTemplate(rules[0].messageTemplateId).body}
          </Box>
            ...
            {_.map(rules[0].parameters, (parameter, index) => {
              return (
                <AvniTextField
                  ...
                  value={parameter}
                  onChange={event =>
                    onVariableChange(index, event.target.value)
                  }
                />
              );
            })}
