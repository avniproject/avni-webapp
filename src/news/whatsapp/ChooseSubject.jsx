...
const SelectSubjectAndConfirm = function({
  subjects,
  onSubjectChosen,
  onCancel,
  confirmLabel
  ...
  return (
    <Box style={{ flexDirection: "column", display: "flex" }}>
      ...
      />
      <Box
        style={{ flexDirection: "row-reverse", display: "flex", marginTop: 20 }}
      >
        <Button
          variant="outlined"
          color="secondary"
...
function ChooseSubject({
  ...
  const displayProgress =
    loading || _.includes([WorkflowStates.Searching], workflowState) || busy;
  const displaySelectSubject =
    !loading && _.includes([WorkflowStates.SearchCompleted], workflowState);
  ...
  return (
    <Box style={{ padding: 20 }}>
      {!loading &&
        _.includes(
          [
            WorkflowStates.Start,
            WorkflowStates.Searching,
            WorkflowStates.SearchError
          ],
          workflowState
        ) && (
          <SearchForm
            ...
            onSearch={searchRequest =>
              searchSubject(setWorkflowState, searchRequest, setSubjects).catch(
                error => {
