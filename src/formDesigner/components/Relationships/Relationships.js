    ...
    () => [
      {
        ...
        Cell: ({ row }) =>
          !row.original.voided ? (
            <a href={`#/appDesigner/relationship/${row.original.id}/show`}>{row.original.name}</a>
          ) : (
      ...
      },
      {
        ...
        enableSorting: false,
        Cell: ({ row }) => {
          const genders = row.original.genders?.map(gender => gender.name) || [];
          return genders.join(", ") || "-";
        }
      }
    ],
        ...
        ? [
            {
              ...
              onClick: (event, row) => navigate(`/appDesigner/relationship/${row.original.id}`),
            ...
            },
            {
              ...
              onClick: (event, row) => {
                const voidedMessage = `Do you really want to delete the relationship ${row.original.name}?`;
          ...
          <div>
            {hasEditPrivilege(userInfo) && (
              <div style={{ float: "right", right: "50px", marginTop: "15px" }}>
                <CreateComponent onSubmit={handleCreateSubmit} name="New Relationship" />
              </div>
            )}
            <AvniMaterialTable
              title=""
