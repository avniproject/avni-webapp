...
const ReportingViews = () => {
    ...
    () => [
      {
        ...
        enableSorting: true,
        Cell: ({ row }) =>
          renderWarning(row.original.viewName, row.original.legacyView)
      }
    ],
    [renderWarning]
  );
  ...
  const fetchData = useCallback(
    ({ page, pageSize, orderBy, orderDirection, globalFilter }) =>
      new Promise(resolve => {
        ...
        if (globalFilter) {
          filteredData = filteredData.filter(view =>
            view.viewName.toLowerCase().includes(globalFilter.toLowerCase())
          );
        }
        if (orderBy) {
        ...
        ? [
            {
              ...
              onClick: (event, row) => {
                const voidedMessage = `Do you really want to delete view ${
                  row.original.viewName
                }?`;
  ...
  const confirmViewCreation = useCallback(() => {
    if (
      window.confirm(
        "Are you sure you want to proceed with view creation/refresh?"
      )
    ) {
      setLoading(true);
      http
      ...
      <DocumentationContainer filename="View.md">
        <Grid container sx={{ justifyContent: "flex-end", mb: 2 }}>
          {hasEditPrivilege(userInfo) && (
            <Grid>
              <CreateComponent
                onSubmit={confirmViewCreation}
                name="Create/Refresh view"
              />
            </Grid>
          )}
        </Grid>
        ...
        />
        <Modal
          onClose={MuiComponentHelper.getDialogClosingHandler()}
          open={loading}
        >
