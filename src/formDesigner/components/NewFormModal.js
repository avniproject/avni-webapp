...
const NewFormModal = ({ name = "", uuid = "", isCloneForm = false }) => {
  ...
  const validateForm = () => {
    ...
    if (_.isNil(formData.formTypeInfo)) errorsList["formTypeInfo"] = "Please select form type.";
  ...
  const formTypes = () => {
    return _.map(
      _.filter(FormTypeEntities.getAllFormTypeInfo(), x => UserInfo.hasFormEditPrivilege(userInfo, x.formType)),
      formTypeInfo => {
        return (
    ...
    <div>
      <form>
        ...
        <FormControl fullWidth margin="dense">
          ...
          <Select id="formTypeInfo" name="formTypeInfo" value={formData.formTypeInfo} onChange={onChangeField} required>
            {formTypes()}
          </Select>
          {errors.formTypeInfo && <FormHelperText error>{errors.formTypeInfo}</FormHelperText>}
        </FormControl>

        <FormControl fullWidth margin="dense">
          <InputLabel htmlFor="name">Name</InputLabel>
          <Input type="text" id="formName" name="name" value={formData.name} onChange={onChangeField} autoComplete="off" fullWidth />
          {errors.name && <FormHelperText error>{errors.name}</FormHelperText>}
        </FormControl>
      </form>

      <Button variant="contained" color="primary" onClick={onSubmitForm} style={{ marginTop: 10 }}>
        Add
      </Button>
