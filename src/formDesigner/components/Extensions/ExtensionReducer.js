...
export const ExtensionReducer = (extension, action) => {
  switch (action.type) {
    case "setLabel":
      return { ...extension, ...updatePropertyAtIndex(extension, "label", action.payload) };
    case "setFileName":
      return { ...extension, ...updatePropertyAtIndex(extension, "fileName", action.payload) };
    case "setScope":
      return {
        ...extension,
    ...
    case "removeSetting":
      ...
      const updatedErrors = filter(extension.errors, ({ key }) => key !== "EMPTY_SETTING");
