...
export const SelectCardsView = ({ dashboardSection, addCards }) => {
  ...
  const onSelectChange = event => {
    const selectedCards = allCards.filter(a =>
      _.some(event, lvPair => a.id === lvPair.value)
    );
      ...
      <Grid container spacing={2}>
        <Grid style={{ zIndex: 2 }} size={10}>
          <Select
            ...
            isSearchable
            options={map(
              WebDashboardSection.getCardsNotAdded(dashboardSection, allCards),
              ({ id, name }) => ({
                label: name,
                value: id
              })
        ...
        <Grid size={2}>
          <Button
            ...
            disabled={cardsToBeAdded.length === 0}
            fullWidth={true}
          >
            Add
          </Button>
