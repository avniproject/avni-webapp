...
export const LocationConcept = props => {
  ...
  useEffect(() => {
    _.isEmpty(lowestAddressLevelTypes) &&
  ...
  function defaultLowestAddressLevelIfSingleHierarchy() {
    if (
      addressLevelTypeHierarchy === undefined ||
      addressLevelTypeHierarchy.size === 0
    )
      return;
  ...
  const updateLowestAddressLevelTypes = (event, uuids) => {
    const lowestAddressLevelTypeUUIDs =
      event !== undefined ? event.target.value : uuids;
