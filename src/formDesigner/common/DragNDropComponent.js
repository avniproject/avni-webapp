...
const DragNDropComponent = ({
  ...
  const handleChange = panel => (event, isExpanded) => {
    console.log("DragNDropComponent: handleChange", { panel, isExpanded, expanded, newExpanded: isExpanded ? panel : null });
  ...
  const safeRenderSummaryText = (data, index, expanded) => {
    ...
    console.warn("DragNDropComponent: renderSummaryText is not a function", {
      ...
      dataList: dataList ? dataList.map(item => ({ uuid: item.uuid, name: item.name })) : null,
      summaryDirection,
      location: "Check the component calling DragNDropComponent"
    });
    if (typeof renderOtherSummary === "function") {
      console.warn("DragNDropComponent: renderOtherSummary is deprecated, please use renderSummaryText and renderSummaryActions");
        ...
        {(provided, snapshot) => (
          <StyledDroppableContainer ref={provided.innerRef} isDraggingOver={snapshot.isDraggingOver} {...provided.droppableProps}>
            {map(orderBy(dataList, "displayOrder"), (data, index) => (
              <Draggable isDragDisabled={expanded !== null} key={data.uuid} draggableId={data.uuid} index={index}>
                {(provided, snapshot) => (
                  <StyledDraggableContainer ref={provided.innerRef} {...provided.draggableProps} isDragging={snapshot.isDragging}>
                    <StyledAccordion key={index} expanded={expanded === "panel" + index} onChange={handleChange("panel" + index)}>
                      ...
                      {renderDetails && <StyledAccordionDetails>{safeRenderDetails(data, index, expanded)}</StyledAccordionDetails>}
                    </StyledAccordion>
