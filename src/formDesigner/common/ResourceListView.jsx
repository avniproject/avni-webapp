...
const ResourceListView = ({
  ...
  const fetchData = useCallback(
    async ({ page, pageSize, orderBy, orderDirection }) => {
      console.log(`[ResourceListView:${resourceName}] fetchData called`, {
        page,
        pageSize,
        orderBy,
        orderDirection
  ...
  useEffect(() => {
    ...
    fetchData({ page: 0, pageSize: 10 })
      ...
      .catch(error => {
        if (isMounted) {
          console.error(
            `[ResourceListView:${resourceName}] Initial fetch failed`,
            error
          );
          setData([]);
          setTotalCount(0);
        ...
        ? [
            {
              ...
              tooltip: `Edit ${title}`,
              onClick: (event, row) =>
                navigate(`/appDesigner/${resourceURLName}/${row.original.id}`),
            ...
            },
            {
              ...
              onClick: (event, row) => {
                const voidedMessage = `Do you really want to delete ${title} ${
                  row.original.name
                }?`;
