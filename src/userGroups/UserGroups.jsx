...
const UserGroups = () => {
  ...
  const showCumulativePrivilegesInfo = () => {
    ...
    return (
      groups !== undefined &&
      groups.length > 1 &&
      groups.filter(group => GroupModel.isEveryoneWithAllPrivileges(group))
        .length > 0 &&
      groups.filter(group => GroupModel.notEveryoneWithoutAllPrivileges(group))
        .length > 0
    );
  };
      ...
      <DocumentationContainer filename={"UserGroup.md"}>
        ...
        {showCumulativePrivilegesInfo() ? (
          <AvniAlert severity={"info"} variant={"outlined"}>
            ...
            <br />
            Since privileges are cumulative across groups, you will need to turn
            off 'All privileges' in the Everyone group for your custom
