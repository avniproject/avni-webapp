...
export const AssignmentToolBar = ({
  ...
  useEffect(() => {
    if (size(props.selectedRows) === 0 && assignmentCriteria.allSelected) {
      dispatch({
        type: "setAction",
        payload: { key: "allSelected", value: false }
      });
    }
  }, [props]);
  ...
  const on1000Selected = () =>
    dispatch({
      type: "setAction",
      payload: { key: "allSelected", value: true }
    });

  return (
    <StyledRoot>
      {selectedRowSize > 0 && (
        <StyledTypography variant="body1">{`Selected ${
          assignmentCriteria.allSelected ? 1000 : selectedRowSize
        } tasks`}</StyledTypography>
      )}
