...
const TaskAssignment = () => {
  ...
  const {
    ...
    taskStatusOptions,
    userOptions
  } = getMetadataOptions(taskMetadata, filterCriteria);
  const applyableTaskStatusesOptions = map(
    applyableTaskStatuses,
    ({ name, id }) => labelValue(name, id)
  );
  ...
  const onActionDone = async () => {
    ...
    const assignmentCriteriaValues = mapValues(assignmentCriteria, (v, k) =>
      includes(["assignToUserIds", "statusId"], k)
      ...
      <Grid container>
        ...
        <Grid size={3}>
          <TaskAssignmentFilter
            ...
            conceptNameAnswerPairs={TaskMetadata.getAllSearchFields(
              taskMetadata
            )}
          />
        </Grid>
