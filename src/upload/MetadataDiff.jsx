...
export const FileDiff = ({ changeReports }) => {
  const renderFieldChange = (changeType, entity, change) => {
    return (
      <Accordion>
        <AccordionSummary>
          <div style={{ color: getColor(changeType), marginLeft: 20 }}>
            {entity}
          </div>
        </AccordionSummary>
        <AccordionDetails>
  ...
  const renderFieldChanges = fieldChanges => {
    return _.map(fieldChanges, change => {
      ...
      const newValue = !_.isNil(change.primitiveValueChange)
        ...
        : !_.isNil(change.objectChangeReport)
        ? change.objectChangeReport.newValue ||
          change.objectChangeReport.fieldChanges
        : getChangeForCollection(
            _.get(change.collectionChangeReport, "changeReports") || []
          );
      const changeDetail = {};
      ...
      if (change.changeType === CHANGE_TYPE.MISSING) {
        ...
        changeDetail.change = !_.isNil(oldValue)
          ? oldValue + " -> " + newValue
          : newValue;
      }
      return changeDetail;
  ...
  const renderChangeReports = changeReports => {
    return _.map(changeReports, cr => {
      ...
      const entityName = changedEntity
        ? changedEntity.name || changedEntity.uuid
        : cr.uuid;
      ...
      switch (cr.changeType) {
        ...
        case CHANGE_TYPE.MODIFIED:
          change = !_.isNil(cr.newValue)
            ? cr.newValue
            : renderFieldChanges(cr.fieldChanges);
          break;
      ...
      return !_.isNil(change)
        ? { changeType: cr.changeType, entity, change }
        : null;
    });
  };
  ...
  return _.map(
    changes,
    c => !_.isNil(c) && renderFieldChange(c.changeType, c.entity, c.change)
  );
};
...
const getChangeForCollection = changeReports => {
  return _.map(
    ...
    cr =>
      cr.changeType +
      " " +
      (_.get(cr.oldValue, "name") || _.get(cr.newValue, "name"))
  ).join(", ");
};
            ...
            <>
              ...
              >
                ...
                <Select
                  ...
                  label="Select Form or Missing Files"
                >
                  {!_.isNil(response.fileChangeReports) &&
              ...
              {selectedForm && response.fileChangeReports[selectedForm] && (
                <div style={{ marginTop: 20 }}>
                  ...
                  <FileDiff
                    changeReports={
                      response.fileChangeReports[selectedForm].changeReports
                    }
                  />
                </div>
              )}
