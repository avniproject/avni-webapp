...
const StyledContainer = styled("div")(({ showIcons, isSelected, paddingLeft }) => ({
  display: "flex",
  flexDirection: "row",
  alignItems: "center",
  height: 30,
...
export const Item = ({ name, onAdd, onDelete, level, disabled, onToggle, isSelected }) => {
  ...
  const renderWithoutLink = extraStyle => <StyledName extraStyle={extraStyle}>{name}</StyledName>;

  return (
    <StyledContainer showIcons={showIcons} isSelected={isSelected} paddingLeft={paddingLeft}>
