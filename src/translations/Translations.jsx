...
const Translations = () => {
  ...
  const organisationConfig = useSelector(
    state => state.translations.onLoad.organisationConfig
  );
  const dashboardData = useSelector(
    state => state.translations.onLoad.dashboardData
  );

  const localeChoices = organisationConfig
    ? getLocales(organisationConfig)
    : [];
    ...
    return http
      ...
      .then(response => {
        ...
        response.data.forEach(data =>
          folder.file(
            data.language + ".json",
            JSON.stringify(data.translationJson, undefined, 2)
          )
        );
        zip
          .generateAsync({ type: "blob" })
          .then(content => saveAs(content, "translations.zip"));
      })
      .catch(error => {
        if (error.response) {
