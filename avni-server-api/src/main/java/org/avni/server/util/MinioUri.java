...
public class MinioUri {
    ...
    /**
     * Creates a new MinioUri by parsing the given string.
     * String will be URL encoded before generating the URI.
     *
     * @param str the URI to parse.
     */
    public MinioUri(final String str) {
        this(str, true);
    }
     ...
     */
    public MinioUri(final String str, final boolean urlEncode) {
        this(URI.create(preprocessUrlStr(str, urlEncode)), urlEncode);
    }

    /**
     * Creates a new MinioUri by wrapping the given {@code URI}.
     *
     * @param uri the URI to wrap
     */
    public MinioUri(final URI uri) {
        this(uri, false);
    }

    private MinioUri(final URI uri, final boolean urlEncode) {
        if (uri == null) {
