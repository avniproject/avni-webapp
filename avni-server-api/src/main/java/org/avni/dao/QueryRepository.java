...
import org.avni.web.response.CustomQueryResponse;
...
public class QueryRepository {
    ...
    private final int MAX_ROW_LIMIT = 2000;

    @Value("${avni.custom.query.timeout}")
    private int QUERY_TIMEOUT_IN_SECONDS;

    private final NamedParameterJdbcTemplate jdbcTemplate;
    private final CustomQueryRepository customQueryRepository;

    @Autowired
    public QueryRepository(@Value("${avni.custom.query.timeout}") int QueryTimeoutInSeconds,
                           NamedParameterJdbcTemplate jdbcTemplate,
                           CustomQueryRepository customQueryRepository) {
        this.jdbcTemplate = jdbcTemplate;
        JdbcTemplate jdbcOperations = (JdbcTemplate) this.jdbcTemplate.getJdbcOperations();
        jdbcOperations.setMaxRows(MAX_ROW_LIMIT);
        jdbcOperations.setQueryTimeout(QueryTimeoutInSeconds);
    ...
    public ResponseEntity<?> runQuery(CustomQueryRequest customQueryRequest) {
        ...
        try {
            ...
            if (errorMessage.equals("ERROR: canceling statement due to user request")) {
                errorMessage = String.format("Query took more than %d seconds to return the result", QUERY_TIMEOUT_IN_SECONDS);
            }
